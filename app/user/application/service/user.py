...
class UserService(UserUseCase):
    ...
    async def get_events_by_id(self, *, user_id: int) -> List[Event] | None:
        try:
    ...
    async def merge_events_by_id(self, *, user_id: int) -> None:
        try:
            # user=await self.repository.get_user_by_id(user_id=user_id)
            events=await self.repository.get_events_by_id(user_id=user_id)
            ...
            for temp_events in non_overlap_events:
                ...
                for event in temp_events:
                    ...
                    invitees.extend(await self.event_repository.get_invitees_by_id(event_id=event.id))
                ...
                # new_event.invitees=invitees
                new_invitees_ids=[invitee.id for invitee in invitees]
                ...
                for invitee_id in new_invitees_ids:
                    await self.add_existing_event_by_eventid(user_id=invitee_id, event_id=new_event.id)
